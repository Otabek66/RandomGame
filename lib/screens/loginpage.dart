...
import 'package:firethree/screens/my_home_page.dart';
...
class _LoginPageState extends State<LoginPage> {
      ...
      body: Center(
        child: Column(
          children: [
            ...
            ElevatedButton(onPressed: loginWithOtp, child: Text("Login with OTP"))
          ],
        ),
      ),
    );
  ...
  Future forgotPassword() async {
    ...
    showSnackBar('Yangi parol uchun link e-mail pochtanggizga jo\'natildi',
        Colors.orange);
  }
    ...
    await auth.verifyPhoneNumber(
      ...
      verificationFailed: (FirebaseAuthException e) {
        if (e.code == "invalid-phone-number") {
          ...
          showSnackBar("Another error type: ${e.message}", Colors.red);
        }
      },
  ...
  Future deleteFromDb() async{
    firestore.collection('users').get().then((documents) {
  ...
  Future listenToDb()async{
    debugPrint("Listening is started !");
    var ref = firestore.collection("users").doc("");
  }

  Future readFromDb() async{
    try{
      var data2 = await firestore.collection("users").orderBy("username")
      .limit(10)
      .startAt(['H']).endAt(['id']).get();
      for (var item in data2.docs) {
        debugPrint(item['username'].toString());
      }
    }catch (e){
      debugPrint("Error while reading: $e");
    }
  }
  ...
  Future writeToDb()async{
    firestore.collection("users").add({
      "username": "Bahrom",
      "password": "445566",
      "image_url": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmJUeQCIV5gK-gudX5l3OIhRcmgnbtGDhExw&usqp=CAU",
